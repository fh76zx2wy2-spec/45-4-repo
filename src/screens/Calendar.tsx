...
import { minutesLabel, DIFFICULTY_LABEL } from '../lib/format';
    ...
    <div className="page stack">
            ...
            return (
              <div className="cal-grid" key={rowStart} role="row">
                {row.map((c) => {
                  ...
                  const cls = ['cal-cell', !c.inMonth ? 'out' : '', base ? 'base' : extra ? 'extra' : '', isToday ? 'today' : ''].join(' ');
                  const label = `${formatHijri(c.iso)} — ${formatGreg(c.iso)}${base ? ' — تمرين' : extra ? ' — جلسة إضافية' : ''}`;
                  return (
                    <button
                      key={c.iso}
                      className={cls}
      ...
      <section className="card">
        ...
        <div className="fstats">
          <div><b className="num">{ms.visits}</b><span>زيارة للنادي</span></div>
        ...
        {ms.visits === 0 && (isPast || isCurrent) && <p className="muted" style={{ fontSize: 13.5, marginTop: 10 }}>لا زيارات مسجّلة في هذا الشهر.</p>}
      </section>
        ...
        {selected && (
          <>
            ...
            {selSessions.length === 0 ? (
              <p className="muted">لا توجد جلسات في هذا اليوم.</p>
            ) : (
