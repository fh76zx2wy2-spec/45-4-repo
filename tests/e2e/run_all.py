...
def full_flow():
    with sync_playwright() as p:
        ...
        page = new_page(b)
        print('\n[1] الدخول والجلسة')
        page.goto(BASE + '/'); page.wait_for_selector('.google-btn')
        ...
        check('لا دخول بلا تحقق: لا تظهر الرئيسية قبل الدخول', page.locator('.home-hello').count() == 0)
        page.click('.google-btn'); page.wait_for_selector('.home-hello', timeout=15000)
        check('الدخول عبر Google ينقل للرئيسية', True)
        tick(page)
        page.screenshot(path=f'{SHOTS}/home-iphone.png')

        # ثبات الجلسة
        page.reload(); page.wait_for_selector('.home-hello', timeout=15000)
        check('الجلسة تبقى بعد إعادة التحميل (بلا شاشة دخول)', page.locator('.google-btn').count() == 0)
        ...
        page.click('.sheet >> text=نعم، اخرج'); page.clock.run_for(4000); page.wait_for_selector('.google-btn', timeout=15000)
...
def google_auth():
    """الدخول بـ Google فقط: الشاشة، حصر المالك، الأذونات، ثبات الجلسة، التجديد، الخروج"""
    with sync_playwright() as p:
        b = launch(p)
        mock('/__admin/reset')
        mock('/__admin/owner?email=ziyad@example.com')
        ctx = b.new_context(**IPHONE, locale='ar-SA', color_scheme='light')
        page = new_page(b, ctx=ctx, time=None)
        print('\n[17] شاشة الدخول: Google فقط')
        page.goto(BASE + '/'); page.wait_for_selector('.google-btn')
        check('زر «الدخول باستخدام Google» واضح', page.locator('.google-btn').inner_text().strip() == 'الدخول باستخدام Google')
        check('لا حقل بريد ولا أي input في شاشة الدخول', page.locator('.login input').count() == 0 and page.locator('input[type=email]').count() == 0)
        check('لا زر «إرسال الرمز» ولا رمز تحقق', page.locator('.login >> text=إرسال').count() == 0 and page.locator('.code-input').count() == 0)
        check('زر Google بارتفاع لمس مريح (≥ 52px)', page.locator('.google-btn').bounding_box()['height'] >= 52)
        page.screenshot(path=f'{SHOTS}/login-google.png')

        print('\n[18] حساب غير المالك يُرفض')
        mock('/__admin/google?email=intruder@example.com')
        page.click('.google-btn'); page.wait_for_selector('.input-error', timeout=15000)
        check('رسالة عربية: الحساب غير مسموح له', 'غير مسموح' in page.locator('.input-error').inner_text(), page.locator('.input-error').inner_text())
        check('لا دخول للمتطفل ولا تظهر الرئيسية', page.locator('.home-hello').count() == 0 and page.locator('.google-btn').count() == 1)
        check('عنوان الصفحة نُظّف من معاملات الخطأ', 'error' not in page.url, page.url)
        check('لم يُنشأ أي مستخدم في الخادم', mock('/__admin/state')['users'] == [])
        page.screenshot(path=f'{SHOTS}/login-denied.png')
        page.reload(); page.wait_for_selector('.google-btn')
        check('الرسالة لا تبقى بعد إعادة التحميل', page.locator('.input-error').count() == 0)

        print('\n[19] إلغاء الدخول من نافذة Google')
        mock('/__admin/google?cancel=1')
        page.click('.google-btn'); page.wait_for_selector('.input-error', timeout=15000)
        check('رسالة الإلغاء هادئة', 'أُلغي' in page.locator('.input-error').inner_text(), page.locator('.input-error').inner_text())

        print('\n[20] الدخول بحساب المالك')
        mock('/__admin/google?email=ziyad@example.com')
        page.click('.google-btn'); page.wait_for_selector('.home-hello', timeout=15000)
        check('حساب المالك يدخل الرئيسية', True)
        st = mock('/__admin/state')
        az = [a for a in st['authorizes'] if a.get('provider') == 'google']
        last = az[-1]
        check('signInWithOAuth بمزوّد google', last.get('provider') == 'google')
        check('النطاقات: openid email profile فقط (لا Gmail)', set(last.get('scopes', '').split()) == {'openid', 'email', 'profile'} and 'gmail' not in json.dumps(last).lower(), last.get('scopes'))
        check('تدفق PKCE مع code_challenge', last.get('code_challenge') and last.get('code_challenge_method', '').lower() == 's256')
        check('لم يُرسل أي بريد/رمز (OTP) إطلاقًا', st['sends'] == [])
        check('عنوان الصفحة بلا رمز OAuth بعد الدخول', 'code=' not in page.url, page.url)
        check('الجلسة محفوظة في الجهاز', page.evaluate("() => !!localStorage.getItem('45-4-auth')"))
        page.screenshot(path=f'{SHOTS}/home-after-google.png')

        print('\n[21] استعادة الجلسة')
        n_auth = len(st['authorizes'])
        page.reload(); page.wait_for_selector('.home-hello', timeout=15000)
        check('إعادة التحميل: تدخل مباشرة بلا شاشة دخول', page.locator('.google-btn').count() == 0)
        page2 = ctx.new_page(); page2.goto(BASE + '/'); page2.wait_for_selector('.home-hello', timeout=15000)
        check('فتح التطبيق من جديد (تبويب جديد) يدخل مباشرة', page2.locator('.google-btn').count() == 0)
        check('لم يُعد التوجيه إلى Google', len(mock('/__admin/state')['authorizes']) == n_auth)
        page2.close()
        # انتهاء الـ access token → يُجدَّد تلقائيًا بالـ refresh token
        before = page.evaluate("() => JSON.parse(localStorage.getItem('45-4-auth')).access_token")
        page.evaluate("() => { const k='45-4-auth'; const s = JSON.parse(localStorage.getItem(k)); s.expires_at = 1; localStorage.setItem(k, JSON.stringify(s)); }")
        page.reload(); page.wait_for_selector('.home-hello', timeout=15000)
        page.wait_for_timeout(800)
        after = page.evaluate("() => JSON.parse(localStorage.getItem('45-4-auth')).access_token")
        check('انتهاء الجلسة القصيرة يُجدَّد تلقائيًا ويبقى الدخول', page.locator('.google-btn').count() == 0 and after != before)

        print('\n[22] تسجيل الخروج')
        page.goto(BASE + '/settings'); page.wait_for_selector('text=تسجيل الخروج')
        page.click('.btn-danger:has-text("تسجيل الخروج")'); page.click('.sheet >> text=نعم، اخرج')
        page.wait_for_selector('.google-btn', timeout=15000)
        check('الخروج يعيد لشاشة الدخول', True)
        check('الجلسة حُذفت من الجهاز', page.evaluate("() => !localStorage.getItem('45-4-auth')"))
        page.reload(); page.wait_for_selector('.google-btn', timeout=15000)
        check('بعد الخروج لا تعود الجلسة عند إعادة التحميل', page.locator('.home-hello').count() == 0)
        page.click('.google-btn'); page.wait_for_selector('.home-hello', timeout=15000)
        check('الدخول مرة أخرى بعد الخروج يعمل', True)

        print('\n[23] حصر المالك في الخادم (RLS المقيِّدة)')
        tok = page.evaluate("() => JSON.parse(localStorage.getItem('45-4-auth')).access_token")
        mock('/__admin/owner?email=someone-else@example.com')
        import urllib.request
        rq = urllib.request.Request(MOCK + '/rest/v1/workout_sessions', headers={'Authorization': 'Bearer ' + tok})
        check('بعد تغيير المالك: الحساب القديم لا يقرأ شيئًا', json.loads(urllib.request.urlopen(rq).read()) == [])
        mock('/__admin/owner?email=ziyad@example.com')
        check('لا أخطاء JavaScript في مسار الدخول', not page.errors, str(page.errors[:3]))
        b.close()

if __name__ == '__main__':
    which = sys.argv[1:] or ['auth', 'flow', 'dark', 'layout']
    for w in which:
        try:
            {'auth': google_auth, 'flow': full_flow, 'dark': settings_dark_and_pwa, 'layout': layouts_and_fonts}[w]()
